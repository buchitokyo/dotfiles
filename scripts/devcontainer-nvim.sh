#!/bin/bash
# devcontainer上でNeovimを起動するスクリプト
# 使い方: .devcontainer/devcontainer-nvim.sh [ファイルパス]
...
# nvimとdotfilesのセットアップ（未セットアップの場合のみ）
devcontainer exec --workspace-folder "$WORKSPACE_FOLDER" \
  bash -c "
    # 基本パッケージインストール
    if ! which unzip > /dev/null 2>&1 || ! which gcc > /dev/null 2>&1; then
      apt-get update && apt-get install -y ripgrep fd-find curl unzip gcc g++ make
    fi
    ...
    if [ ! -d \"\$HOME/dotfiles\" ]; then
      ...
      # dotfilesにinstall.shがあれば実行、なければnvim設定をシンボリックリンク
